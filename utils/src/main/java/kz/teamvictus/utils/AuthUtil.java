    ...
    public static Boolean checkPasswordHash(String password, String hash) throws InternalException {
        try {
            if (!hash.equalsIgnoreCase(sha256(password))) {
                throw IE_HELPER.generate(ErrorCode.ErrorCodes.AUTH_ERROR, "Ошибка авторизации.");
            }
            ...
            LOGGER.error(e.getMessage(), e);
            throw IE_HELPER.generate(ErrorCode.ErrorCodes.SYSTEM_ERROR, "Ошибка авторизации.", e);
        }
