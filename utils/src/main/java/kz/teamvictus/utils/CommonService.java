...
public class CommonService {
    ...
    private static final Gson GSON = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss").serializeNulls().create();
    private static final Gson GSON_DATETIME = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").serializeNulls().create();
    private static final Gson GSON_DATE_ONLY = new GsonBuilder().setDateFormat("yyyy-MM-dd").serializeNulls().create();
//    InternalExceptionHelper ieHelper = new InternalExceptionHelper(this.getClass().getCanonicalName());
    ...
    protected String success(List<?> data, long dataSize) {
        Map answer = new HashMap<>();
        answer.put("status", SUCCESS);
        answer.put("data", data);
        answer.put("count", dataSize);
        return GSON.toJson(answer);
    }
